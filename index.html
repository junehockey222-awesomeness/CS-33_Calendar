...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: Arial, sans-serif; }
        ...
        .main-header { display: flex; align-items: center; background-color: white; padding: 15px 30px; border-radius: 50px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); z-index: 10; position: relative; }
        ...
        .mobile-settings-icon {
            ...
            cursor: pointer;
            color: #555;
            margin-left: 15px; /* Spacing from title */
            padding: 5px;
            line-height: 1;
        }
        ...
        .clean-mode-legend { 
            ...
            z-index: 30; /* Higher z-index to ensure it sits on top */
            min-width: 200px;
        ...
        @media (max-width: 768px) {
            ...
            .mobile-settings-icon { display: block; }
            
            /* HIDE ADMIN SETTINGS BUTTON ON MOBILE */
            #adminLoginBtn { display: none !important; }
        ...
        .admin-controls-container.visible { display: block; }
...
<body>
    ...
    <div id="settingsModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <button id="publicExportBtn" class="settings-btn-toggle inactive" style="background-color: #28a745; color: white; border: none;">Export Calendar</button>
            <button id="adminLoginBtn" class="settings-btn-toggle inactive" style="background-color: #6c757d; color: white; border: none;">Admin Settings</button>
            
            <div id="adminControlsContainer" class="admin-controls-container">
                <h4 style="margin-bottom:15px; color:#d9534f; text-transform:uppercase; font-size:0.9rem;">Admin Controls</h4>
                <button id="toggleEditModeBtn" class="settings-btn-toggle inactive">Enter Edit Mode</button>
                <button id="setSemesterBtn" class="settings-btn-toggle inactive" style="background-color: #17a2b8; color: white; border: none;">Set Semester Start / End</button>
                <button id="exportDataBtn" class="settings-btn-toggle inactive" style="background-color: #6610f2; color: white; border: none;">Export All Data (Backup)</button>
                ...
                <button id="changePinBtn" class="settings-btn-toggle inactive" style="background-color: #343a40; color: white; border: none;">Change PIN</button>
            </div>
        </div>
    </div>
    
    <div id="publicExportModal" class="modal-overlay">
        <div class="modal-content" style="max-width: 450px;">
            <button class="close-modal-btn" id="closePublicExportBtn">&times;</button>
            <h3 class="modal-title">Export Calendar</h3>
            <p style="color:#666; margin-bottom:15px; font-size:0.9rem;">Select event types to include in your download.</p>
            
            <div class="filter-section" style="border-bottom:1px solid #eee; padding-bottom:15px; margin-bottom:15px;">
                <h4 style="color:#003087; margin-bottom:10px;">Select Events:</h4>
                <div class="designator-list">
                    <label><input type="checkbox" value="Birthday" class="export-filter-check" checked> Birthday</label>
                    <label><input type="checkbox" value="Briefing" class="export-filter-check" checked> Briefing</label>
                    <label><input type="checkbox" value="Checkpoint" class="export-filter-check" checked> Checkpoint</label>
                    <label><input type="checkbox" value="Fitness Test" class="export-filter-check" checked> Fitness Test</label>
                    <label><input type="checkbox" value="Formation" class="export-filter-check" checked> Formation</label>
                    <label><input type="checkbox" value="Holiday" class="export-filter-check" checked> Holiday</label>
                    <label><input type="checkbox" value="IC Competition" class="export-filter-check" checked> IC Competition</label>
                    <label><input type="checkbox" value="Morale Event" class="export-filter-check" checked> Morale Event</label>
                    <label><input type="checkbox" value="Squadron Time" class="export-filter-check" checked> Squadron Time</label>
                    <label><input type="checkbox" value="Tasker" class="export-filter-check" checked> Tasker</label>
                    <label><input type="checkbox" value="Training" class="export-filter-check" checked> Training</label>
                    <label><input type="checkbox" value="Other" class="export-filter-check" checked> Other</label>
                </div>
            </div>
            
            <div style="display:flex; flex-direction:column; gap:10px;">
                <button id="exportAppleBtn" class="settings-btn-toggle" style="background-color: #333; color: white; border: none; margin-bottom:0;">
                    Download for Apple
                </button>
                <button id="exportGoogleBtn" class="settings-btn-toggle" style="background-color: #4285F4; color: white; border: none; margin-bottom:0;">
    ...
    <div class="top-container">
        ...
        <div class="main-header">
            ...
            <button id="mobileSettingsBtn" class="mobile-settings-icon">⚙️</button>
        </div>
        ...
        <div class="clean-mode-legend" id="cleanModeLegend">
            ...
            <div class="legend-column">
                ...
                <div class="legend-item"><div class="legend-dot" style="background: #fd7e14;"></div> Non-Mandatory</div>
            </div>
        </div>
    </div>
    ...
    <script type="module">
        ...
        let currentPin = null; // No longer hardcoded.
        ...
        ['monthYear','calendarGrid','prevBtn','nextBtn','weekViewBtn','mainHeaderTitle','mainHeaderLogo',
         ...
         'settingsModal', 'closeSettingsBtn', 'toggleEditModeBtn', 'openEventListBtn', 'setSemesterBtn', 'adminLoginBtn', 'adminControlsContainer', 'publicExportBtn',
         ...
         'publicExportModal', 'closePublicExportBtn', 'exportAppleBtn', 'exportGoogleBtn',
         'mobileSettingsBtn'
        ...
        const exportFilterChecks = document.querySelectorAll('.export-filter-check');
        ...
        // 2. Handle Android/Chrome (Automatic Prompt)
        window.addEventListener('beforeinstallprompt', (e) => {
            e.preventDefault();
            deferredPrompt = e;
            // Show button if on mobile and not already installed
        ...
        // 3. Handle iOS (Manual Instructions)
        if (isIos && !isStandalone && window.innerWidth <= 768) {
            // Show button immediately for iOS since the event above won't fire
            els.installContainer.style.display = 'flex';
        ...
        window.addEventListener('resize', () => {
            ...
            // Show if we have a prompt waiting OR if it's iOS
            if ((deferredPrompt || isIos) && window.innerWidth <= 768) {
                els.installContainer.style.display = 'flex';
            } else {
                els.installContainer.style.display = 'none';
            }
            ...
            try {
                try {
                    ...
                    if (pinDoc.exists()) { 
                        ...
                        console.log("No security PIN set in database. Admin locked.");
                    }
        ...
        function renderDayView() {
            ...
            const todayStr = getFormattedDate(new Date());
            if (dateKey === todayStr) {
                els.mainHeaderTitle.style.color = "#ff0000"; // Bright Red for Today
            } else {
                els.mainHeaderTitle.style.color = "#333"; // Normal Color otherwise
            }

            const options = { weekday: 'long' };
            els.monthYear.innerText = currentDate.toLocaleDateString('en-US', options);
            els.monthYear.style.color = ""; // Reset to normal color (removed red override)
        ...
        function createEventChip(evt) {
            ...
            if (evt.hasOwnProperty('mandatory') && !evt.mandatory) {
                // Non-Mandatory is Orange
                chip.style.backgroundColor = "#fd7e14"; 
                chip.style.color = "white";
                chip.style.border = "none";
            } else if (evt.tentative) {
                // Keep tentative style if mandatory but tentative (though legend removed *)
        ...
        els.settingsBtn.addEventListener('click', () => {
            // No longer check PIN here. Open public menu.
            els.settingsModal.style.display = 'flex';
            updateSettingsUI();
            
            // Ensure Admin Button is visible on Desktop if not unlocked/locked logic applies standardly
            if(!isSettingsUnlocked) els.adminLoginBtn.style.display = 'block';
        });

        // Mobile Settings Button Handler
        els.mobileSettingsBtn.addEventListener('click', () => {
             els.settingsModal.style.display = 'flex';
             updateSettingsUI();
             
             // Hide Admin Login Button for Mobile Public Users
             els.adminLoginBtn.style.display = 'none';
        });

        // Admin Login Button in Public Menu
        els.adminLoginBtn.onclick = () => {
             els.settingsModal.style.display = 'none'; // Temporarily hide main settings
             els.pinInput.value = ""; 
        ...
        function checkPin() {
            if (currentPin && els.pinInput.value === currentPin) { 
                isSettingsUnlocked = true; 
                els.pinModal.style.display = 'none'; 
                els.settingsModal.style.display = 'flex'; // Re-open settings
                updateSettingsUI(); 
            } 
            else { alert("Incorrect PIN."); els.pinInput.value = ""; els.pinInput.focus(); }
        }
        els.submitPinBtn.onclick = checkPin;
        els.cancelPinBtn.onclick = () => {
            els.pinModal.style.display = 'none';
            els.settingsModal.style.display = 'flex'; // Go back to public settings
        };
        ...
        function updateSettingsUI() {
            ...
            if (isSettingsUnlocked) {
                els.adminLoginBtn.style.display = 'none';
                els.adminControlsContainer.classList.add('visible');
            } else {
                // Default state - will be overridden by mobile click handler if needed
                els.adminLoginBtn.style.display = 'block';
                els.adminControlsContainer.classList.remove('visible');
            }
        ...
        // --- PUBLIC EXPORT CALENDAR ---
        els.publicExportBtn.onclick = () => {
             els.settingsModal.style.display = 'none';
             els.publicExportModal.style.display = 'flex';
        };
        els.closePublicExportBtn.onclick = () => els.publicExportModal.style.display = 'none';

        function generateExport(format) {
             const selectedTypes = [];
             exportFilterChecks.forEach(cb => { if (cb.checked) selectedTypes.push(cb.value); });
             
             if(selectedTypes.length === 0) { alert("Please select at least one event type."); return; }
             
             const exportEvents = events.filter(e => selectedTypes.includes(e.designator));
             
             if (exportEvents.length === 0) { alert("No events found with selected types."); return; }

             let icsContent = "";
             
             if (format === 'vcs') {
                 // vCalendar 1.0 (VCS)
                 icsContent = "BEGIN:VCALENDAR\nVERSION:1.0\nPRODID:-//CS40 Calendar//EN\n";
                 exportEvents.forEach(evt => {
                     // Safety checks for corrupt data
                     if (!evt.date || typeof evt.date !== 'string') return;
                     
                     let desc = "";
                     if (evt.notes && typeof evt.notes === 'string') {
                         desc = evt.notes.replace(/\n/g, '\\n').replace(/;/g, '\\;').replace(/,/g, '\\,');
                     }
                     let summary = evt.name || "Event";
                     
                     let startTimeSafe = (evt.startTime && typeof evt.startTime === 'string') ? evt.startTime.replace(':','') + '00' : '000000';
                     let endTimeSafe = (evt.endTime && typeof evt.endTime === 'string') ? evt.endTime.replace(':','') + '00' : '235900';

                     let dtStart = evt.date.replace(/-/g, '') + 'T' + startTimeSafe;
                     let dtEnd = evt.date.replace(/-/g, '') + 'T' + endTimeSafe;
                 ...
                 downloadFile(icsContent, "cs40_calendar.vcs", "text/x-vcalendar");
             } 
             else {
                 // iCalendar 2.0 (ICS) - Apple
                 icsContent = "BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//CS40 Calendar//EN\nCALSCALE:GREGORIAN\n";
                 exportEvents.forEach(evt => {
                     // Safety check
                     if (!evt.date || typeof evt.date !== 'string') return;
                     ...
                     if (evt.startTime && typeof evt.startTime === 'string' && evt.startTime.trim() !== "") {
                         allDay = false;
                         dtStart += `T${evt.startTime.replace(':','')}00`;
                         if (evt.endTime && typeof evt.endTime === 'string' && evt.endTime.trim() !== "") {
                            dtEnd += `T${evt.endTime.replace(':','')}00`;
                         } else {
                            // Default to +1 hour if no end time
                            let parts = evt.startTime.split(':');
                            if (parts.length >= 2) {
                                let h = parseInt(parts[0]) + 1;
                                let m = parts[1];
                                if (h >= 24) { h = 23; m = '59'; } // Clamp to end of day
                                dtEnd += `T${String(h).padStart(2,'0')}${m}00`;
                            } else {
                                dtEnd += `T235900`;
                            }
                         }
                     ...
                     if (evt.location) icsContent += `LOCATION:${evt.location}\n`;
                     if (evt.notes) icsContent += `DESCRIPTION:${evt.notes.replace(/\n/g, '\\n')}\n`;
                     icsContent += "END:VEVENT\n";
                 });
                 icsContent += "END:VCALENDAR";
                 downloadFile(icsContent, "cs40_calendar.ics", "text/calendar");
             }
        }
        ...
        els.exportAppleBtn.onclick = () => generateExport('ics');
        els.exportGoogleBtn.onclick = () => generateExport('vcs');
        ...
        // --- EXPORT DATA LOGIC (ADMIN BACKUP) ---
        ...
        window.onclick = (e) => { 
            ...
            if (e.target == els.publicExportModal) els.publicExportModal.style.display = "none";
        };
